...
@interface XLsn0wCycleImg () <UICollectionViewDataSource, UICollectionViewDelegate>
...
+ (instancetype)cycleWithFrame:(CGRect)frame imageURLArray:(NSArray *)imageURLArray {
    XLsn0wCycleImg *cycleScrollView = [[self alloc] initWithFrame:frame];
    cycleScrollView.imageURLArray = [NSMutableArray arrayWithArray:imageURLArray];
...
- (void)setImageURLArray:(NSArray *)imageURLArray {
    ...
    [_imageURLArray enumerateObjectsUsingBlock:^(NSString * obj, NSUInteger idx, BOOL * stop) {
        NSString *urlString;
...
- (void)addPageNumber {
    ...
    _pageView.backgroundColor = [UIColor xlsn0w_hexString:@"#000000" alpha:0.5];;
    
    _pageTitle = [[UILabel alloc] init];
    [_pageView addSubview:_pageTitle];
    [_pageTitle mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(15);
        make.centerY.mas_equalTo(self->_pageView);
        make.height.mas_equalTo(20);
        make.width.mas_equalTo(250);
    }];
    _pageTitle.textColor = AppWhiteTextColor;
    _pageTitle.font = [UIFont systemFontOfSize:14];
    _pageTitle.text = [self.pageTitleArray firstObject];
    
    _pageNumber = [[UILabel alloc] init];
    [_pageView addSubview:_pageNumber];
...
-(NSMutableAttributedString *)firstString:(NSString *)firstString firstColor:(UIColor *)firstColor secondString:(NSString *)secondString secondColor:(UIColor *)secondColor{
    ...
    NSDictionary * firstAttributes = @{ NSFontAttributeName:[UIFont systemFontOfSize:14],NSForegroundColorAttributeName:firstColor};
...
- (void)showPageNumber:(NSInteger)index {
    self.pageNumber.attributedText = [self firstString:[NSString stringWithFormat:@"%ld/", index+1]
                                            firstColor:AppThemeColor
                                          secondString:[NSString stringWithFormat:@"%ld", _imageURLArray.count]
                                           secondColor:AppWhiteTextColor];
}

- (void)showPageTitle:(NSInteger)index {
    self.pageTitle.text = self.pageTitleArray[index];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.pageTitle.text = self.pageTitleArray[itemIndex];

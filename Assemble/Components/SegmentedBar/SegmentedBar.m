...
@interface SegmentedBar () <UIScrollViewDelegate>
...
@property (nonatomic, assign) SegmentedBarIndicatorType indicatorType;
...
- (instancetype)initWithFrame:(CGRect)frame
                       titles:(NSArray *)titlesArr
                     delegate:(id<SegmentTitleViewDelegate>)delegate
                indicatorType:(SegmentedBarIndicatorType)incatorType {
...
- (void)initWithProperty {
    self.itemMargin = 20;
    self.selectIndex = 0;
    self.titleNormalColor = AppBlackTextColorAlpha;
    self.titleSelectColor = AppThemeColor;
    self.titleFont = [UIFont systemFontOfSize:15];
    
    self.indicatorColor = self.titleSelectColor;
    self.indicatorCustomWidth = 100;
...
- (void)layoutSubviews {
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [SegmentedBar getWidthWithString:self.titlesArr[idx] font:titleFont] + self.itemMargin;
    ...
    else
    {
        for (NSString *title in self.titlesArr) {
            CGFloat itemBtnWidth = [SegmentedBar getWidthWithString:title font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    }
...
- (void)moveIndicatorView:(BOOL)animated {
    ...
    CGFloat indicatorWidth = [SegmentedBar getWidthWithString:self.titlesArr[self.selectIndex] font:titleFont];
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (self.indicatorType) {
            case SegmentedBarIndicatorTypeEqualTitle:
                ...
                break;
            case SegmentedBarIndicatorTypeCustom:
                self.indicatorView.center = CGPointMake(selectBtn.center.x, CGRectGetHeight(self.scrollView.bounds) - 1);
                self.indicatorView.bounds = CGRectMake(0, 0, self.indicatorCustomWidth, 2);
                break;
            case SegmentedBarIndicatorTypeNone:
...
- (void)setIndicatorCustomWidth:(CGFloat)indicatorCustomWidth {
    _indicatorCustomWidth = indicatorCustomWidth;
    [self setNeedsLayout];
    [self layoutIfNeeded];
}

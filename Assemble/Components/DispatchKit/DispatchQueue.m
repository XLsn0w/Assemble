...
static DispatchQueue *highPriorityGlobalQueue;
static DispatchQueue *lowPriorityGlobalQueue;
static DispatchQueue *backgroundPriorityGlobalQueue;
...
+ (DispatchQueue *)highPriorityGlobalQueue {
    
    return highPriorityGlobalQueue;
}

+ (DispatchQueue *)lowPriorityGlobalQueue {
    
    return lowPriorityGlobalQueue;
}

+ (DispatchQueue *)backgroundPriorityGlobalQueue {
...
+ (void)initialize {
     ...
     */
    if (self == [DispatchQueue self])  {
        
        mainQueue                     = [DispatchQueue new];
        globalQueue                   = [DispatchQueue new];
        highPriorityGlobalQueue       = [DispatchQueue new];
        lowPriorityGlobalQueue        = [DispatchQueue new];
        backgroundPriorityGlobalQueue = [DispatchQueue new];
...
- (void)execute:(dispatch_block_t)block inGroup:(DispatchGroup *)group {
    
    NSParameterAssert(block);
    dispatch_group_async(group.dispatchGroup, self.dispatchQueue, block);
}

- (void)notify:(dispatch_block_t)block inGroup:(DispatchGroup *)group {

...
- (void)setModel:(TZAssetModel *)model {
    ...
    if (self.useCachedImage && model.cachedImage) {
        ...
        int32_t imageRequestID = [[TZImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
//                [self hideProgressView];
            ...
            if ([self.representedAssetIdentifier isEqualToString:[[TZImageManager manager] getAssetIdentifier:model.asset]]) {
                ...
                // HLLog(@"this cell is showing other asset");
...
//                [self hideProgressView];
        ...
        if (imageRequestID && self.imageRequestID && imageRequestID != self.imageRequestID) {
            [[PHImageManager defaultManager] cancelImageRequest:self.imageRequestID];
            // HLLog(@"cancelImageRequest %d",self.imageRequestID);
        }
        self.imageRequestID = imageRequestID;
    }
    self.selectPhotoButton.selected = model.isSelected;
    self.selectImageView.image = self.selectPhotoButton.isSelected ? self.photoSelImage : [UIImage imageNamed:@"photo_def_photoPickerVc"];
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    self.selectImageView.image = sender.isSelected ? self.photoSelImage : [UIImage imageNamed:@"photo_def_photoPickerVc"];
...
- (void)layoutSubviews {
    ...
    _selectImageView.frame = CGRectMake(self.width - 27, 3, 24, 24);
    ...
    _bottomView.frame = CGRectMake(0, self.height - 17, self.width, 17);

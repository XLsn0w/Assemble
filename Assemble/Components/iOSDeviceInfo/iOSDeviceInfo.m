...
+(NSArray *)executablePathAndMD5Value{
    ...
    NSString* md5 = [iOSDeviceInfo fileMD5:appPath];
    if(nil == md5)
    md5 = @"";
...
+ (NSDictionary *)deviceInfo {
    ...
    NSString *deviceType = [iOSDeviceInfo deviceTypeDetail];
    ...
    [dic setObject:[iOSDeviceInfo cpuArchitectures] forKey:@"CPUArchitecture"];
    [dic setObject:[iOSDeviceInfo getBundleID] forKey:@"BundleID"];
    [dic setObject:[iOSDeviceInfo getLocalAppVersion] forKey:@"AppVersion"];
    [dic setObject:[iOSDeviceInfo getApplicationName] forKey:@"AppName"];
    [dic setObject:[iOSDeviceInfo getDeviceIPAdress] forKey:@"IP"];
    [dic setObject:@([iOSDeviceInfo getBatteryLevel]) forKey:@"BatteryLevel"];
    [dic setObject:[iOSDeviceInfo getBatteryState] forKey:@"BatteryState"];
    return dic;
}
...
+ (NSString *)getDeviceDisplayMetrics {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
        ...
        if ([iOSDeviceInfo deviceInfo_iPhoneX]||
            [[iOSDeviceInfo deviceTypeDetail] isEqualToString:@"iPhone XS"])
        {
            DisplayMetrics = @"2436*1125";
        }
        if ([[iOSDeviceInfo deviceTypeDetail] isEqualToString:@"iPhone XR"])
        {
            DisplayMetrics = @"1792*828";
        }
        if ([[iOSDeviceInfo deviceTypeDetail]isEqualToString:@"iPhone XS Max"]) {
...
//获取设备型号
//后续更新机型可根据https://www.theiphonewiki.com/wiki/Models#iPhone添加
...
-(NSString*)getMainBundleMD5WithFlag:(NSInteger)flag
{
    ...
    while (self.dirFileList.count>0) {
        ...
        NSString *fileMD5 = [iOSDeviceInfo fileMD5:fileFullPathName];
    ...
    if(flag == 1)
    {
        ...
        [outFile closeFile];
        //读文件MD5
        MD5 = [iOSDeviceInfo fileMD5:filePath];
        //取得MD5后，马上删除文件
        [defaultManager removeItemAtPath:filePath error:nil];
    ...
    }else
    {
        ...
        //读数据MD5
        MD5 = [iOSDeviceInfo dataMD5:allFilesMD5string];
    }

...
@implementation DispatchDBManager
...
+ (instancetype)shareManager:(NSString *)fl_dbName{
    ...
    static DispatchDBManager *instance = nil;
...
- (void)fl_createTable:(Class)modelClass complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    // 创建完毕不关闭数据库，执行完其他操作会关闭
    [self fl_createTable:modelClass autoCloseDB:NO complete:complete];
}

- (void)fl_insertModel:(id)model complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    [self fl_insertModel:model autoCloseDB:YES complete:complete];
}

- (void)fl_insertModel:(id)model autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
...
- (void)fl_searchModel:(Class)modelClass byID:(NSString *)FLDBID complete:(void(^)(DispatchDBManager *manager,id model))complete{
    [self fl_searchModel:modelClass byID:FLDBID autoCloseDB:YES complete:complete];
}

- (void)fl_searchModelArr:(Class)modelClass complete:(void(^)(DispatchDBManager *manager,NSArray *modelArr))complete{
    [self fl_searchModelArr:modelClass autoCloseDB:YES complete:complete];
}

- (void)fl_searchModelArr:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(DispatchDBManager *manager,NSArray *modelArr))complete{
...
- (void)fl_modifyModel:(id)model byID:(NSString *)FLDBID complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    [self fl_modifyModel:model byID:FLDBID autoCloseDB:YES complete:complete];
}

- (void)fl_modifyModel:(id)model byID:(NSString *)FLDBID autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
...
- (void)fl_dropTable:(Class)modelClass complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    [self fl_dropTable:modelClass autoCloseDB:YES complete:complete];
}

- (void)fl_dropTable:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    __block BOOL success = true;
    __weak typeof(self) weakSelf = self;
    FLDISPATCH_ASYNC_GLOBAL(^{
        //把任务包装到事务里
...
- (void)fl_deleteAllModel:(Class)modelClass complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    [self fl_deleteAllModel:modelClass autoCloseDB:YES complete:complete];
}

- (void)fl_deleteAllModel:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    __block BOOL success = true;
    __weak typeof(self) weakSelf = self;
...
- (void)fl_deleteModel:(Class)modelClass byId:(NSString *)FLDBID complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    [self fl_deleteModel:modelClass byId:FLDBID autoCloseDB:YES complete:complete];
}

- (void)fl_deleteModel:(Class)modelClass byId:(NSString *)FLDBID autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    __block BOOL success = true;
    __weak typeof(self) weakSelf = self;
    FLDISPATCH_ASYNC_GLOBAL(^{
        //把任务包装到事务里
...
- (void)fl_isExitTable:(Class)modelClass complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
 ...
 *  指定的表是否存在
 */
- (void)isExitTable:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    __block BOOL success = true;
    __weak typeof(self) weakSelf = self;
    //把任务包装到事务里
    FLDISPATCH_ASYNC_GLOBAL(^{
 ...
 *  创表
 */
- (void)fl_createTable:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
 ...
 *  插入数据
 */
- (void)fl_insertSingleModel:(id)model inAsync:(BOOL)inAsync autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    __block BOOL success = true;
    __weak typeof(self) weakSelf = self;
    if (inAsync) {
        FLDISPATCH_ASYNC_GLOBAL(^{
...
- (void)fl_insertModelArr:(NSArray *)modelArr autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(DispatchDBManager *manager, BOOL flag))complete{
    ...
    FLDISPATCH_ASYNC_GLOBAL(^{
        ...
        for (NSInteger index = 0; index < modelArr.count; index ++) {
            ...
            if (index == modelArr.count - 1) {
                [strongSelf fl_insertSingleModel:model inAsync:NO autoCloseDB:autoCloseDB complete:^(DispatchDBManager *manager, BOOL flag) {
                    if (complete) {
                        FLDISPATCH_ASYNC_MAIN(^{
 ...
 *  查询
 */
- (void)fl_searchModel:(Class)modelClass byID:(NSString *)FLDBID autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(DispatchDBManager *manager,id model))complete{
    __block BOOL success = true;
    __weak typeof(self) weakSelf = self;
    //把任务包装到事务里
    FLDISPATCH_ASYNC_GLOBAL(^{

...
+ (void)launchADWithFrame:(CGRect)frame aDduration:(NSInteger)duration aDImageUrl:(NSString *)imageUrl hideSkipButton:(BOOL)hideSkip launchAdClickBlock:(DDLaunchAdClickBlock)aDClickBlock {
    [[LaunchADManager alloc] initWithFrame:frame aDduration:duration aDImageUrl:imageUrl hideSkipButton:false launchAdClickBlock:^{
...
- (UIImageView *)setUpAdImageView {
    if (self.adImageView == nil) {
        ...
        if ([RegularExpression checkURL:self.aDImageUrl]) {
            if ([idString isEqualToString:@"gif"]) {
                NSData *urlData = [NSData dataWithContentsOfURL:[NSURL URLWithString:self.aDImageUrl]];
                [self.adImageView addSubview:[[LaunchADGIFView alloc] initWithFrame:self.adFrame gifImageData:urlData]];
            } else {
            ...
            if ([idString isEqualToString:@"gif"]) {
                ...
                [self.adImageView addSubview:[[LaunchADGIFView alloc] initWithFrame:self.adFrame gifImageData:localData]];
            } else {
...
- (void)dispath_timer {
    ...
    dispatch_source_set_event_handler(self.timer, ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            duration--;
            if (duration > -1) {
                [self.skipButton setTitle:[NSString stringWithFormat:@"%ld 跳过", duration+1] forState:UIControlStateNormal];
            } else {

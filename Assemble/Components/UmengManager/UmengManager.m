...
static UmengManager *_instance = nil;
...
- (void)shareWebToPlatform:(UMSocialPlatformType)platformType
    ...
    UMShareWebpageObject *shareObject = [UMShareWebpageObject shareObjectWithTitle:title
                                                                             descr:descr
                                                                         thumImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:thumImageUrl]]]];
    //设置网页地址
    ...
    [[UMSocialManager defaultManager] shareToPlatform:platformType messageObject:messageObject currentViewController:vc completion:^(id data, NSError *error) {
        if (error) {
            XLsn0wLog(@"分享报错===== %@*********",error);
        }else{
            if ([data isKindOfClass:[UMSocialShareResponse class]]) {
                UMSocialShareResponse *resp = data;
                //分享结果消息
                XLsn0wLog(@"分享结果消息======= %@", resp.message);
                //第三方原始返回的数据
                XLsn0wLog(@"第三方原始返回的数据====== %@",resp.originalResponse);
                
            }else{
                XLsn0wLog(@"response data is %@",data);
            }
        }
    }];
}

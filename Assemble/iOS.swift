/*
 ...
 (swift是extension延展)Category是类别，一般情况用分类好，用Category去重写类的方法，仅对本Category有效，不会影响到其他类与原有类的关系。
 ...
 比如说，当在程序A里面一段内存被成功申请完成之后，那么这个计数器就从0变成1(我们把这个过程叫做alloc)，然后程序B也需要使用这个内存，那么计数器就从1变成了2(我们把这个过程叫做retain)。紧接着程序A不再需要这段内存了，那么程序A就把这个计数器减1(我们把这个过程叫做release);程序B也不再需要这段内存的时候，那么也把计数器减1(这个过程还是release)。当系统(也就是Foundation)发现这个计数器变 成员了0，那么就会调用内存回收程序把这段内存回收(我们把这个过程叫做dealloc)。
 
 ***顺便提一句，如果没有Foundation, CF指针由人工管理,手动的CFRetain和CFRelease来管理,那么维护计数器，释放内存等等工作需要你手工来完成。
 ...
 答：category和extension的不同在于 后者可以添加属性。另外后者添加的方法是必须要实现的。
 ...
 对于kvc机制如何通过key寻找到value： KVC支持属性和实例变量
 ...
 FMDBQueue :  自定义安全线程   创建多线程 然后按顺序执行  必须同时多线程同时访问造成死锁
 ...
 1
 + (void)transitionWithView:(UIView *)view duration:(NSTimeInterval)duration options:(UIViewAnimationOptions)options animations:(void (^)(void))animations completion:(void (^)(BOOL finished))completion NS_AVAILABLE_<a class="keylink" href="http://www.2cto.com/kf/yidong/iphone/" target="_blank">IOS</a>(4_0);
 操作的函数执行部分
 ...
 NSMutableArray *Array = [[NSArray alloc] init]; 后，需要 [aArray release];
 ...
 * 通过计算校验和，进行传输数据的完整性检查。
 ...
 答：数据库框架，对sqllite的数据操作进行了封装，使用着可把精力都放在sql语句上面。 fmdbQueue 线程安全
